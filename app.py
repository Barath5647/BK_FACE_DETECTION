...
# --- Make sure Keras knows how to (de)serialize your mixed‑precision policy ---
#   The saved model refers to class_name="DTypePolicy", so we register Policy under that name.
...
# Image dims must match your training
IMG_SIZE = (64, 64)

# Load your model inside a scope that knows DTypePolicy → Policy
with custom_object_scope({"DTypePolicy": Policy}):
    model = load_model("cnn_model.h5", compile=False)

def preprocess_image(img: Image.Image) -> np.ndarray:
    """Resize, normalize, and batchify."""
    arr = np.array(img)
...
if uploaded:
    ...
    st.write(f"**Predicted:** {labels[idx]}  — probability {preds[idx]:.2f}")

    # Optional: detect open/closed eye
